...
public class BaseReg {
    ...
    private static Block registerBlock(String name, Block block, boolean burnable, int fuelTime, int burnChance, int burnSpread) {
        ...
        Block hold = Registry.register(Registry.BLOCK, new Identifier(DelChoco.DELCHOCO_ID, name), block);
        if (burnable) { FlammableBlockRegistry.getDefaultInstance().add(hold, burnChance, burnSpread); }
        return hold;
    }
    private static Item registerBlockItem(String name, Block block) {
        return Registry.register(Registry.ITEM, new Identifier(DelChoco.DELCHOCO_ID, name), new BlockItem(block, new FabricItemSettings().group(DelChoco.DELCHOCO_ITEMS)));
    }
    ...
    public static void registerModItems() { DelChoco.LOGGER.info("Registering "+DelChoco.DELCHOCO_ID +" Items"); }
}